...
INPUT_CSV = PROJECT_ROOT / "all_results_w_sweep.csv"
OUTPUT_AGG = PROJECT_ROOT / "w_sweep_aggregated.csv"
...
for rate in rates:
    ...
    plt.title(f"p99 vs W (rate={rate})")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(PROJECT_ROOT / f"plot_p99_vs_w_rate_{rate}.png", dpi=150)
    plt.close()
    ...
    plt.title(f"throughput vs W (rate={rate})")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(PROJECT_ROOT / f"plot_tps_vs_w_rate_{rate}.png", dpi=150)
    plt.close()
    ...
    plt.title(f"p95 vs W (rate={rate})")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(PROJECT_ROOT / f"plot_p95_vs_w_rate_{rate}.png", dpi=150)
    plt.close()
    ...
    plt.title(f"mean latency vs W (rate={rate})")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(PROJECT_ROOT / f"plot_mean_latency_vs_w_rate_{rate}.png", dpi=150)
    plt.close()
