...
$RunDir = Join-Path $ProjectRoot "results\q_sweep_block_w4_work5000"
New-Item -ItemType Directory -Force -Path $RunDir | Out-Null

$AllResultsCsv = Join-Path $RunDir "all_results_q_sweep_block_w4_work5000.csv"
...
$Queues  = @(1000, 5000, 10000)
$Rates   = @(600000, 700000)
$Repeats = 5

Set-Location $ProjectRoot

Write-Host "SCRIPT VERSION: Q SWEEP"
...
for ($qi = 0; $qi -lt $Queues.Count; $qi++) {
    $Q = $Queues[$qi]
    ...
    for ($ri = 0; $ri -lt $Rates.Count; $ri++) {
        ...
        for ($rep = 1; $rep -le $Repeats; $rep++) {
            $ExpId = "q_${Q}_rate_${Rate}_rep_${rep}"
